...
router.get('/real', authenticate, async (req, res) => {
  try {
    ...
    const startWeek = Math.ceil((startDate - semesterStart) / (7 * 24 * 60 * 60 * 1000));
    const endWeek = Math.ceil((endDate - semesterStart) / (7 * 24 * 60 * 60 * 1000));
    ...
    let rescheduledQuery = `
      ...
      AND se.event_date BETWEEN $${queryParams.length + 1} AND $${queryParams.length + 2}
    `;
    ...
    if (req.user.role !== 'admin') {
      rescheduledQuery += ` AND EXISTS (
        ...
        WHERE csub.user_id = $${queryParams.length + 1} AND csub.course_id = se.course_id
      )`;
    ...
    console.log('📊 Found rescheduled courses:', rescheduledResult.rows.map(r => `${r.course_name} on ${r.event_date}`));
    console.log('📊 Found default courses:', defaultResult.rows.map(r => r.course_name));
    console.log('📅 Date range being queried:', startDate.toISOString().split('T')[0], 'to', endDate.toISOString().split('T')[0]);
    ...
    const rescheduledCourses = new Set(rescheduledResult.rows.map(event => event.course_id));
    ...
    allEvents.forEach(event => {
      ...
      if (event.event_date) {
        // Rescheduled event - use actual date
    ...
    console.log('📊 Final unique courses by date:', Object.keys(eventsByDate).map(date => 
      `${date}: [${eventsByDate[date].map(e => e.course_name).join(', ')}]`
    ));
...
router.post('/update', authenticate, requireAdminOrKomting, async (req, res) => {
  try {
    const { courseId, newDate, newStartTime, newEndTime, newRoomId, weekNumber, meetingNumber } = req.body;
    ...
    console.log('🔄 Reschedule request:', { courseId, newDate, newStartTime, newEndTime, newRoomId, weekNumber });
    ...
    const calculatedWeek = Math.ceil((eventDate - semesterStart) / (7 * 24 * 60 * 60 * 1000));
    ...
    try {
      ...
      try {
        await client.query('ALTER TABLE schedule_events ADD COLUMN IF NOT EXISTS week_number INTEGER');
        await client.query('ALTER TABLE schedule_events ADD COLUMN IF NOT EXISTS meeting_number INTEGER');
      } catch (e) {
      ...
      if (timeConflictCheck.rows.length > 0) {
        ...
        return res.status(409).json({
          error: 'Time conflict',
          details: `Time slot conflicts with ${timeConflictCheck.rows[0].course_name} in room ${timeConflictCheck.rows[0].room_number || 'TBA'}`
        });
      }
      ...
      console.log(`🗑️ Deleted ${deleteResult.rows.length} existing events for course ${courseId}, week ${finalWeekNumber}`);
      ...
      const result = await client.query(`
        ...
        RETURNING *
      `, [courseId, newRoomId, newDate, newStartTime, newEndTime, req.user.id, finalWeekNumber, meetingNumber]);

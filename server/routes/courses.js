...
const courseSchema = Joi.object({
  ...
  default_start_time: Joi.string()
    .pattern(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/)
    .required(),
  default_end_time: Joi.string()
    .pattern(/^([01]?[0-9]|2[0-3]):[0-5][0-9]$/)
    .required(),
...
router.get("/", async (req, res) => {
  try {
    ...
    if (search) {
      query += ` AND (c.course_name ILIKE $${
        queryParams.length + 1
      } OR c.course_code ILIKE $${queryParams.length + 1})`;
...
router.get("/my/subscriptions", authenticateToken, async (req, res) => {
  try {
    // Accept several possible token payload shapes
    const user_id = req.user?.id || req.user?.sub || req.user?.user_id;

    if (!user_id) {
      console.error(
        "Get subscriptions error: missing user id in token",
        req.user
      );
      return res.status(401).json({
    ...
    `,
      ...
      // Fallback for DBs where courses.lecturer_id column doesn't exist
      console.warn(
        "Subscriptions query with lecturer JOIN failed, falling back to basic query:",
        err.message
      );
      result = await pool.query(
        `
    ...
    // Map null lecturer_name to empty string for consistency

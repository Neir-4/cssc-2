...
async function createNextWeekEvents() {
  try {
    const client = await pool.connect();
    
    // Next week: December 15-19, 2025
    const monday = new Date('2025-12-15');
    const friday = new Date('2025-12-19');
    
    console.log('📅 Creating events for next week:', monday.toISOString().split('T')[0], 'to', friday.toISOString().split('T')[0]);
    ...
    for (const course of courses.rows) {
      // Calculate the date for this course in next week
    ...
    console.log('\n📊 Created events for next week:', verifyEvents.rows.length);

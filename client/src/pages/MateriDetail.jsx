...
const MateriDetail = () => {
    ...
    const loadCourseData = async () => {
      try {
        ...
        const course = subscriptions.find(
          (sub) => sub.name?.toLowerCase().replace(/\s+/g, "-") === courseId
        );
  ...
  const handleFileUpload = async (meetingNumber, file) => {
    try {
      ...
      const response = await apiService.uploadMaterial(
        ...
        <div className="space-y-4">
          {meetings.map((meeting) => (
            ...
            >
              ...
              {/* Upload Form */}
              {showUpload[meeting.id] && user?.role === "komting" && (
                ...
                <div className="text-center py-8 text-gray-500">
                  ...
                  {user?.role === "komting" && (
                    <p className="text-sm mt-1">
                      Klik tombol Upload untuk menambah materi
                    </p>
                  )}
                </div>
              ) : (

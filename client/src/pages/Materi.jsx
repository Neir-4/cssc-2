...
const Materi = () => {
  ...
  useEffect(() => {
    const loadSubscriptions = async () => {
      try {
        ...
        // load material counts (non-blocking)
        try {
          const countsResp = await apiService.getMaterialsCounts();
          setMaterialCounts(countsResp.counts || {});
        } catch (err) {
          console.warn("Could not load material counts:", err);
        }
      } catch (error) {
  ...
  const materials = mySubscriptions.map((sub, index) => ({
    id:
      sub.name?.toLowerCase().replace(/\s+/g, "-") || `course-${sub.course_id}`,
    name: sub.name,
    course_code: sub.course_code,
    credits: sub.credits || 3,
    pengajar: sub.lecturer_name || "Dosen Pengampu",
    meetings: 16, // Standard 16 meetings per semester
    semester: sub.semester || "Ganjil",
    fileCount: materialCounts[sub.course_id]?.total || 0,
    lastUploaded: materialCounts[sub.course_id]?.last_uploaded || null,
    ...
    <div className="min-h-screen bg-gray-50 pt-24 pb-16 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col items-center text-center mb-12">
          <span className="text-amber-500 font-bold tracking-wider uppercase text-sm mb-2">
            Academic Resources
          </span>
          ...
          <p className="text-gray-500 max-w-2xl text-lg mb-8">
            Pilih mata kuliah untuk mengunduh materi, slide presentasi, dan
            catatan dari dosen pengampu.
          </p>
                ...
                <div className="p-5 flex flex-col grow justify-between">
                  <div>
                        ...
                        <div className="flex items-center text-xs font-medium text-gray-500 bg-gray-50 px-2 py-1 rounded-full">
                          ...
                          >
                            <path
                              ...
                              strokeLinejoin="round"
                              strokeWidth={2}
                              d="M8 17l3-3 3 3m0 0V8a4 4 0 10-8 0v9"
                            />
                          </svg>
                          {item.fileCount} file
                        </div>
                    ...
                    <div className="flex items-start gap-2 mt-4">
                      <svg
                        className="w-4 h-4 text-gray-400 mt-1 shrink-0"
                        fill="none"
                        viewBox="0 0 24 24"
                        stroke="currentColor"
                      >

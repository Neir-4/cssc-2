...
const Profile = () => {
  ...
  const loadCoursesAndSubscriptions = async () => {
    try {
      ...
      console.log(
        "📋 My subscriptions:",
        subsResponse.subscriptions?.length || 0
      );
  ...
  const handleSubscribe = async (courseId) => {
    try {
      ...
      showAlert({ type: "error", message: "Gagal berlangganan mata kuliah" });
    }
  };
  ...
  const handleUnsubscribe = async (courseId) => {
    try {
      ...
      showAlert({ type: "error", message: "Gagal berhenti berlangganan" });
    }
  };

  const isSubscribed = (courseId) => {
    const subscribed = mySubscriptions.some(
      (sub) => sub.course_id === courseId
    );
    return subscribed;
  };
  ...
  const filteredCourses = (
    subscriptionMode ? allCourses : mySubscriptions
  ).filter(
    (course) =>
      ...
      <div className="pt-20 min-h-screen bg-[#f5f5f0] flex justify-center items-start px-4 sm:px-6 lg:px-8 pb-20">
        <div className="bg-white shadow-xl rounded-2xl w-full max-w-2xl mt-6 sm:mt-10 border border-gray-100 overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-4">
              ...
              <div className="text-center sm:text-left">
                ...
                <p className="text-white/90 text-sm sm:text-base">
                  {user.role === "komting" ? "Komting" : "Mahasiswa"}
                </p>
              </div>
            </div>
          </div>
                ...
                <div className="mb-6">
                  <div className="relative">
                    ...
                    <input
                      ...
                      placeholder="Cari mata kuliah, kode, atau nama pengampu..."
                      value={searchTerm}
                    ...
                    {displayedCourses.length === 0 ? (
                      <p className="text-gray-500 text-center py-4">
                        {searchTerm
                          ...
                          : "Belum berlangganan mata kuliah apapun"}
                      </p>
                    ) : (
                      <>
                        ...
                        {hasMoreCourses && (
                          <div className="text-center pt-4">
                            ...
                            >
                              Tampilkan{" "}
                              {Math.min(2, filteredCourses.length - showCount)}{" "}
                              lagi
                            </button>
                          </div>
                        )}
          ...
          <div className="px-6 sm:px-8 py-6 sm:py-8 border-t border-gray-200 mb-16">
            <h2 className="text-xl font-bold mb-6 text-[#1e1e1e]">
              Informasi Personal
            </h2>
            ...
            <div className="space-y-4">
              ...
              <ProfileField
                label="Peran"
                value={user.role === "komting" ? "Komting" : "Mahasiswa"}
                readOnly
              />

              {}
              <ProfileField
                label={user.role === "komting" ? "NIP" : "NIM"}
...
const ProfileField = ({
  ...
  return (
    <div className="flex flex-col sm:flex-row sm:items-center border-b border-gray-100 pb-3">
      ...
      {!editMode || readOnly ? (
        <span className="text-[#1e1e1e] text-sm sm:text-base break-all">
          {value || "-"}
        </span>
      ) : (
        <input

...
const Pengumuman = () => {
  ...
  const getRoleStyle = (role) => {
    ...
    // treat any 'dosen' or other as pengampu/anggota fallback
    return {
  ...
  const canCreateAnnouncement =
    user?.role?.toString().toLowerCase() === "komting";

  // Filter announcements by subscription
  const filteredAnnouncements = announcements.filter((announcement) => {
    // Komting dapat lihat semua pengumuman
    if (user?.role?.toString().toLowerCase() === "komting") return true;
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 gap-4">
          <div>
            ...
            <p className="text-gray-500 text-sm">
              Informasi terbaru seputar perkuliahan dan akademik.
            </p>
          </div>
                    ...
                    >
                      ...
                      <div className="flex justify-between items-start mb-3">
                        <div className="flex items-center gap-3">
                          {/* Avatar */}
                          <div
                            className={`w-10 h-10 rounded-full flex items-center justify-center text-lg ${style.badge}`}
                          >
                            {style.icon}
                          </div>
                          <div>
                            <div className="flex items-center gap-2">
                              ...
                              <span
                                className={`text-[10px] uppercase tracking-wider font-bold px-2 py-0.5 rounded-full ${style.badge}`}
                              >
                                {item.authorRole}
                              </span>
                            </div>
...
const CreateAnnouncementModal = ({
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showAlert({
        type: "error",
        message: "Gagal membuat pengumuman. Silakan coba lagi.",
      });
    } finally {
      setLoading(false);
    }
